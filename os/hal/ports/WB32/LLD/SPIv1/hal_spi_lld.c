...
#define QSPI_RX_DMA_CHANNEL                                                 \
  WB32_DMA_GETCHANNEL(WB32_SPI_QSPI_RX_DMA_STREAM,                        \
                       WB32_QSPI_RX_DMA_CHN)

#define QSPI_TX_DMA_CHANNEL                                                 \
  WB32_DMA_GETCHANNEL(WB32_SPI_QSPI_TX_DMA_STREAM,                        \
                       WB32_QSPI_TX_DMA_CHN)

#define SPIM2_RX_DMA_CHANNEL                                                 \
  WB32_DMA_GETCHANNEL(WB32_SPI_SPIM2_RX_DMA_STREAM,                        \
                       WB32_SPIM2_RX_DMA_CHN)

#define SPIM2_TX_DMA_CHANNEL                                                 \
  WB32_DMA_GETCHANNEL(WB32_SPI_SPIM2_TX_DMA_STREAM,                        \
                       WB32_SPIM2_TX_DMA_CHN)

#define SPIS1_RX_DMA_CHANNEL                                                 \
  WB32_DMA_GETCHANNEL(WB32_SPI_SPIS1_RX_DMA_STREAM,                        \
                       WB32_SPIS1_RX_DMA_CHN)

#define SPIS1_TX_DMA_CHANNEL                                                 \
  WB32_DMA_GETCHANNEL(WB32_SPI_SPIS1_TX_DMA_STREAM,                        \
                       WB32_SPIS1_TX_DMA_CHN)

#define SPIS2_RX_DMA_CHANNEL                                                 \
  WB32_DMA_GETCHANNEL(WB32_SPI_SPIS2_RX_DMA_STREAM,                        \
                       WB32_SPIS2_RX_DMA_CHN)

#define SPIS2_TX_DMA_CHANNEL                                                 \
  WB32_DMA_GETCHANNEL(WB32_SPI_SPIS2_TX_DMA_STREAM,                        \
                       WB32_SPIS2_TX_DMA_CHN)
#endif
/*===========================================================================*/
...
void spi_lld_start(SPIDriver *spip) {
  ...
  if (spip->state == SPI_STOP) {
    ...
    if (&SPIDQ == spip) {
      ...
      RCC_APB1PeriphClockCmd( RCC_APB1Periph_BMX1 | RCC_APB1Periph_QSPI, ENABLE );
    ...
    if (&SPIDS1 == spip) {
      ...
      RCC_APB1PeriphClockCmd( RCC_APB1Periph_BMX1 | RCC_APB1Periph_SPIS1, ENABLE );
...
void spi_lld_stop(SPIDriver *spip) {
  ...
  if (spip->state == SPI_READY) {
    ...
    if (&SPIDQ == spip){
      ...
      RCC_APB1PeriphClockCmd( RCC_APB1Periph_QSPI, DISABLE );}
#endif
#if WB32_SPI_USE_SPIM2
    if (&SPIDM2 == spip){
      nvicDisableVector(SPIM2_IRQn);
      /* Disable the SPIM2 clock */
      RCC_APB2PeriphClockCmd( RCC_APB2Periph_SPIM2, DISABLE );}
#endif
#if WB32_SPI_USE_SPIS1
    if (&SPIDS1 == spip){
      nvicDisableVector(SPIS1_IRQn);
      /* Disable the SPIS1 clock */
      RCC_SPIS1CLKCmd(DISABLE);
      RCC_APB1PeriphClockCmd( RCC_APB1Periph_SPIS1, DISABLE );}
#endif
#if WB32_SPI_USE_SPIS2
    if (&SPIDS2 == spip){
      nvicDisableVector(SPIS2_IRQn);
      /* Disable the SPIS2 clock */
      RCC_SPIS2CLKCmd(DISABLE);
      RCC_APB2PeriphClockCmd( RCC_APB2Periph_SPIS2, DISABLE );}
#endif
  }
}
