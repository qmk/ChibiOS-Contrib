 ...
 */
#if !defined(WB32_USB_LOW_POWER_ON_SUSPEND) || defined(__DOXYGEN__)
 ...
 * @notapi
 */
#define usb_lld_get_frame_number(usbp) (uint16_t)((WB32_USB->FRAMEH << 8) | WB32_USB->FRAMEL)
...
#define usb_lld_connect_bus(usbp)                                                     \
  ...
  {                                                                                   \
    ...
    GPIOA->CFGMSK = ~(GPIO_Pin_11 | GPIO_Pin_12);                                     \
    GPIOA->CURRENT = 0x3 * 0x55555555U;                                               \
    _pal_lld_setgroupmode(GPIOA, GPIO_Pin_11 | GPIO_Pin_12, GPIO_MODE_AF | GPIO_AF3); \
    USB->INTRUSBE = USB_INTRUSBE_RSTIE | USB_INTRUSBE_RSUIE;                          \
  } while (false)
#endif
...
#define usb_lld_disconnect_bus(usbp)                                            \
  do                                                                            \
  {                                                                             \
    /* Enable BMX1, GPIOA c\lock */                                             \
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_BMX1 | RCC_APB1Periph_GPIOA, ENABLE); \
    /* Configure PA11 and PA12 as input mode */                                 \
    _pal_lld_setgroupmode(GPIOA, GPIO_Pin_11 | GPIO_Pin_12, GPIO_MODE_IN);      \
    /* Configure PA12(D+) as open-drain output mode and output low level */     \
    _pal_lld_setgroupmode(GPIOA, GPIO_Pin_12, GPIO_MODE_OUT | GPIO_OTYPE_OD);   \
    pal_lld_clearport(GPIOA, GPIO_Pin_12);                                      \
  } while (false)
