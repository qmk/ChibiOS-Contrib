...
static void _wbusb_ep0in(USBDriver *usbp, usbep_t ep) {
  ...
  case USB_EP0_IN_TX:
    ...
    WB32_USB->CSR0 |= USB_CSR0_DATAEND;
...
static size_t usb_packet_read_to_buffer(usbep_t ep, uint8_t *buf) {
  ...
  if (ep_num != 0)
  {
    n =  (size_t)((WB32_USB->OUTCOUNTH << 8) | WB32_USB->OUTCOUNTL);
  }
  else
  {
    n =  WB32_USB->COUNT0;
...
static void usb_serve_EPin_handler(USBDriver *usbp,usbep_t ep) {
  ...
  ControlReg = WB32_USB->INCSR1;

  if (!(ControlReg & USB_INCSR1_FIFONE))
  {
    isp->txcnt += isp->txlast;
    n = isp->txsize - isp->txcnt;
...
/**
 * @brief   Common ISR code, serves the core-related interrupts.
 *
 * @param[in] usbp      pointer to the @p USBDriver object
 *
 * @notapi
 */
static void usb_serve_core(USBDriver *usbp) {
  ...
  if ((IntrUSB & USB_INTRUSB_RSUIS) != 0) {
    /* Resume interrupt */
    _usb_wakeup(usbp);
  }

  /* USB bus SUSPEND condition handling.*/
  if ((IntrUSB & USB_INTRUSB_SUSIS) != 0) {
    /* Suspend interrupt */
    _usb_suspend(usbp);
  }

  /* SOF handling.*/
  if ((IntrUSB & USB_INTRUSB_SOFIS) != 0) {
    /* sof hook function */
...
void usb_lld_start(USBDriver *usbp) {
  ...
  if (usbp->state == USB_STOP) {
    ...
    if (&USBD1 == usbp) {
      ...
      RCC_USBCLKConfig(RCC_USBCLKSource_MAINCLK_1Div5, ENABLE);
#elif (WB32_USBPRE == WB32_USBPRE_DIV1)
      RCC_USBCLKConfig(RCC_USBCLKSource_MAINCLK_Div1, ENABLE);
#elif (WB32_USBPRE == WB32_USBPRE_DIV2)
      RCC_USBCLKConfig(RCC_USBCLKSource_MAINCLK_Div2, ENABLE);
#elif (WB32_USBPRE == WB32_USBPRE_DIV3)
      RCC_USBCLKConfig(RCC_USBCLKSource_MAINCLK_Div3, ENABLE);
#else
...
void usb_lld_reset(USBDriver *usbp) {
  ...
  /* Post reset initialization.*/
   WB32_USB->INTRINE  = 0X00; /* Disable  in all endpoint interrupt */
   WB32_USB->INTROUTE = 0x00; /* Disable out all endpoint interrupt */
  ...
  // WB32_USB->INTRUSBE = (USB_INTRUSBE_RSTIE | USB_INTRUSBE_SUSIE | USB_INTRUSBE_RSUIE);
}
...
void usb_lld_init_endpoint(USBDriver *usbp, usbep_t ep) {
      ...
      case USB_EP_MODE_TYPE_ISOC:
        {
          ...
          if ((epcp->in_state != NULL) || (epcp->out_state != NULL))
          {
            ...
            WB32_USB->OUTCSR2 =(USB_OUTCSR2_AUTOCLR |
                                USB_OUTCSR2_ISO);
  ...
  else
  {
    ...
    {
      // CTRL Mode
      WB32_USB->INTRINE = USB_INTRINE_EP0E;  //Enable in endpoint 0 interrupt
      WB32_USB->INTROUTE = 0x00;             //Disable out all endpoint interrupt
    }
  }
...
void usb_lld_clear_out(USBDriver *usbp, usbep_t ep) {
  ...
  {
    WB32_USB->CSR0 = USB_CSR0_SENTSTALL;
  }
}
