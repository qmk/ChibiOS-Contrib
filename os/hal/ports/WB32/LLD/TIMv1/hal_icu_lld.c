...
static bool icu_lld_wait_edge(ICUDriver *icup) {
  ...
  if (icup->config->channel == ICU_CHANNEL_1) {
    ...
    while (((sr = icup->tim->SR) &
            (WB32_TIM_SR_CC1IF | WB32_TIM_SR_UIF)) == 0)
      ;
  }
  else {
    /* Waiting for an edge.*/
    while (((sr = icup->tim->SR) &
            (WB32_TIM_SR_CC2IF | WB32_TIM_SR_UIF)) == 0)
      ;
  }
...
#error "WB32_TIM1_CC_HANDLER not defined"
#endif
/**
 * @brief   TIM1 compare interrupt handler.
 *
...
void icu_lld_start(ICUDriver *icup) {
  ...
  if (icup->config->channel == ICU_CHANNEL_1) {
    ...
    if (icup->config->mode == ICU_INPUT_ACTIVE_HIGH)
      icup->tim->CCER = WB32_TIM_CCER_CC1E |
                        WB32_TIM_CCER_CC2E | WB32_TIM_CCER_CC2P;
    else
